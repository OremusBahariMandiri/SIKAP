        ...
        <div class="col-md-12">
            <div class="card shadow">
                <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                    <span class="fw-bold">Edit Dokumen Legal</span>
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('dokLegal.update', $dokLegal) }}" method="POST" enctype="multipart/form-data" id="dokLegalForm">
                        @csrf
                        @method('PUT')
                        <input type="hidden" name="IdKode" value="{{ old('IdKode', $dokLegal->IdKode) }}">
                                ...
                                <div class="card h-100 border-info">
                                    ...
                                    <div class="card-body">
                                        <div class="form-group mb-3">
                                            <label for="NoRegDok" class="form-label fw-bold">Nomor Register Dokumen <span class="text-danger">*</span></label>
                                            <input type="text" class="form-control @error('NoRegDok') is-invalid @enderror" id="NoRegDok" name="NoRegDok" value="{{ old('NoRegDok', $dokLegal->NoRegDok) }}" placeholder="Masukan No Register Dokumen" required>
                                            @error('NoRegDok')
                                        ...
                                        <div class="form-group mb-3">
                                            <label for="perusahaan_id" class="form-label fw-bold">Perusahaan <span class="text-danger">*</span></label>
                                            <select class="form-select @error('perusahaan_id') is-invalid @enderror" id="perusahaan_id" name="perusahaan_id" required>
                                                ...
                                                @foreach ($perusahaans as $id => $nama)
                                                    <option value="{{ $id }}" {{ old('perusahaan_id', $dokLegal->perusahaan_id) == $id ? 'selected' : '' }}>{{ $nama }}</option>
                                                @endforeach
                                            </select>
                                            @error('perusahaan_id')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                            <input type="hidden" id="DokPerusahaan" name="DokPerusahaan" value="{{ old('DokPerusahaan', $dokLegal->DokPerusahaan) }}">
                                        </div>

                                        <div class="form-group mb-3">
                                            <label for="kategori_id" class="form-label fw-bold">Kategori Dokumen <span class="text-danger">*</span></label>
                                            <select class="form-select @error('kategori_id') is-invalid @enderror" id="kategori_id" name="kategori_id" required>
                                                ...
                                                @foreach ($kategoris as $id => $kategori)
                                                    <option value="{{ $id }}" {{ old('kategori_id', $dokLegal->kategori_id) == $id ? 'selected' : '' }}>{{ $kategori }}</option>
                                                @endforeach
                                            </select>
                                            @error('kategori_id')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                            <input type="hidden" id="KategoriDok" name="KategoriDok" value="{{ old('KategoriDok', $dokLegal->KategoriDok) }}">
                                        </div>

                                        <div class="form-group mb-3">
                                            <label for="jenis_id" class="form-label fw-bold">Jenis Dokumen <span class="text-danger">*</span></label>
                                            <select class="form-select @error('jenis_id') is-invalid @enderror" id="jenis_id" name="jenis_id" required>
                                                ...
                                                @foreach ($jenisDoks as $id => $jenis)
                                                    <option value="{{ $id }}" {{ old('jenis_id', $dokLegal->jenis_id) == $id ? 'selected' : '' }}>{{ $jenis }}</option>
                                                @endforeach
                                            </select>
                                            @error('jenis_id')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                            <input type="hidden" id="JenisDok" name="JenisDok" value="{{ old('JenisDok', $dokLegal->JenisDok) }}">
                                        </div>

                                        <div class="form-group mb-3">
                                            <label for="PeruntukanDok" class="form-label fw-bold">Peruntukan Dokumen <span class="text-danger">*</span></label>
                                            <input type="text" class="form-control @error('PeruntukanDok') is-invalid @enderror" id="PeruntukanDok" name="PeruntukanDok" value="{{ old('PeruntukanDok', $dokLegal->PeruntukanDok) }}" placeholder="Masukan Peruntukan Dokumen" required>
                                            @error('PeruntukanDok')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="form-group mb-3">
                                            <label for="DokAtasNama" class="form-label fw-bold">Atas Nama <span class="text-danger">*</span></label>
                                            <input type="text" class="form-control @error('DokAtasNama') is-invalid @enderror" id="DokAtasNama" name="DokAtasNama" value="{{ old('DokAtasNama', $dokLegal->DokAtasNama) }}" placeholder="Masukan Nama" required>
                                            @error('DokAtasNama')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                ...
                                <div class="card h-100 border-info">
                                    ...
                                    <div class="card-body">
                                        <div class="form-group mb-3">
                                            ...
                                            <div class="bg-light p-2 rounded">
                                                <div class="form-check form-check-inline">
                                                    <input class="form-check-input jenis-masa-berlaku @error('JnsMasaBerlaku') is-invalid @enderror" type="radio" name="JnsMasaBerlaku" id="tetap" value="Tetap" {{ old('JnsMasaBerlaku', $dokLegal->JnsMasaBerlaku) == 'Tetap' ? 'checked' : '' }} required>
                                                    <label class="form-check-label" for="tetap">
                                                        <i class="fas fa-infinity text-primary me-1"></i>Tetap
                                                    </label>
                                                </div>
                                                <div class="form-check form-check-inline">
                                                    <input class="form-check-input jenis-masa-berlaku @error('JnsMasaBerlaku') is-invalid @enderror" type="radio" name="JnsMasaBerlaku" id="perpanjangan" value="Perpanjangan" {{ old('JnsMasaBerlaku', $dokLegal->JnsMasaBerlaku) == 'Perpanjangan' ? 'checked' : '' }}>
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group mb-3">
                                                    <label for="TglTerbitDok" class="form-label fw-bold">Tanggal Terbit <span class="text-danger">*</span></label>
                                                    <div class="input-group">
                                                        <span class="input-group-text"><i class="fas fa-calendar"></i></span>
                                                        <input type="date" class="form-control tanggal-input @error('TglTerbitDok') is-invalid @enderror" id="TglTerbitDok" name="TglTerbitDok" value="{{ old('TglTerbitDok', $dokLegal->TglTerbitDok ? $dokLegal->TglTerbitDok->format('Y-m-d') : '') }}" required>
                                                        @error('TglTerbitDok')
                                                            <div class="invalid-feedback">{{ $message }}</div>
                                                        @enderror
                                                    </div>
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group mb-3 tgl-berakhir-group">
                                                    ...
                                                    <div class="input-group">
                                                        ...
                                                        <input type="date" class="form-control tanggal-input @error('TglBerakhirDok') is-invalid @enderror" id="TglBerakhirDok" name="TglBerakhirDok" value="{{ old('TglBerakhirDok', $dokLegal->TglBerakhirDok ? $dokLegal->TglBerakhirDok->format('Y-m-d') : '') }}">
                                        ...
                                        <div class="form-group mb-3">
                                            ...
                                            <input type="text" class="form-control bg-light" id="MasaBerlaku" name="MasaBerlaku" value="{{ old('MasaBerlaku', $dokLegal->MasaBerlaku) }}" readonly>
                                            <div class="form-text text-muted"><i class="fas fa-info-circle me-1"></i>Masa berlaku akan dihitung otomatis</div>
                                        </div>

                                        <div class="form-group mb-3 tgl-pengingat-group">
                                            <label for="TglPengingat" class="form-label fw-bold">Tanggal Peringatan</label>
                                            <div class="input-group">
                                                <span class="input-group-text"><i class="fas fa-bell"></i></span>
                                                <input type="date" class="form-control tanggal-input @error('TglPengingat') is-invalid @enderror" id="TglPengingat" name="TglPengingat" value="{{ old('TglPengingat', $dokLegal->TglPengingat ? $dokLegal->TglPengingat->format('Y-m-d') : '') }}">
                                                @error('TglPengingat')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                        ...
                                        <div class="form-group mb-3">
                                            ...
                                            <input type="text" class="form-control bg-light" id="MasaPengingat" name="MasaPengingat" value="{{ old('MasaPengingat', $dokLegal->MasaPengingat) }}" readonly>
                                            <div class="form-text text-muted"><i class="fas fa-info-circle me-1"></i>Masa pengingat akan dihitung otomatis</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group mb-3">
                                                    ...
                                                    <div class="form-text text-muted"><i class="fas fa-info-circle me-1"></i>Format: PDF, JPG, PNG, DOC, DOCX, XLS, atau XLSX</div>

                                                    @if ($dokLegal->FileDok)
                                                        <div class="mt-2 d-flex align-items-center">
                                                            <span class="badge bg-success me-2"><i class="fas fa-check"></i></span>
                                                            <span>File saat ini: {{ $dokLegal->FileDok }}</span>
                                                            <a href="{{ route('dokLegal.download', $dokLegal) }}" class="btn btn-sm btn-outline-primary ms-2">
                                                ...
                                                <div class="form-group mb-3">
                                                    <label class="form-label fw-bold">Status Berlaku Dokumen <span class="text-danger">*</span></label>
                                                    <div class="bg-light p-2 rounded">
                                                        <div class="form-check form-check-inline">
                                                            <input class="form-check-input @error('StsBerlakuDok') is-invalid @enderror" type="radio" name="StsBerlakuDok" id="berlaku" value="Berlaku" {{ old('StsBerlakuDok', $dokLegal->StsBerlakuDok) == 'Berlaku' ? 'checked' : '' }} required>
                                                            <label class="form-check-label" for="berlaku">
                                                                <i class="fas fa-check-circle text-success me-1"></i>Berlaku
                                                            </label>
                                                        </div>
                                                        <div class="form-check form-check-inline">
                                                            <input class="form-check-input @error('StsBerlakuDok') is-invalid @enderror" type="radio" name="StsBerlakuDok" id="tidak_berlaku" value="Tidak Berlaku" {{ old('StsBerlakuDok', $dokLegal->StsBerlakuDok) == 'Tidak Berlaku' ? 'checked' : '' }}>
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group mb-3 h-100">
                                                    <label for="KetDok" class="form-label fw-bold">Keterangan</label>
                                                    <textarea class="form-control @error('KetDok') is-invalid @enderror" id="KetDok" name="KetDok" rows="5" placeholder="Masukkan keterangan tambahan jika ada">{{ old('KetDok', $dokLegal->KetDok) }}</textarea>
                                                    @error('KetDok')
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                    @enderror
                                                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.getElementById('perusahaan_id').value) {
        ...
        document.getElementById('DokPerusahaan').value = perusahaanSelect.options[perusahaanSelect.selectedIndex].text;
    }

    if (document.getElementById('kategori_id').value) {
        const kategoriSelect = document.getElementById('kategori_id');
        document.getElementById('KategoriDok').value = kategoriSelect.options[kategoriSelect.selectedIndex].text;
    }
    ...
    function toggleFieldsVisibility() {
        ...
        if (jenisMasaBerlaku === 'Tetap') {
            ...
            if (!tglBerakhirLabel.innerHTML.includes('*')) {
                tglBerakhirLabel.innerHTML = tglBerakhirLabel.innerHTML + ' <span class="text-danger">*</span>';
            }
